    ...
    <div class="dashboard-shell">
        ...
        <main class="dashboard-main">
            ...
            <section class="sales-dashboard">
                @php
                    $selectedUnitFinancialAccounts = $selectedUnit
                        ? $financialAccountsByHost->get($selectedUnit->host_id, collect())
                        : collect();
                @endphp
                        ...
                        <div class="unit-finance-config-grid">
                            ...
                            <details class="unit-finance-panel">
                                ...
                                <form method="POST" action="{{ route('sales.units.costs.store', $selectedUnit) }}" class="unit-finance-form">@csrf
                                    ...
                                    <label><span>Payment status</span><div class="finance-control"><span aria-hidden="true"><i class="fa-solid fa-circle-check"></i></span><select name="status" required data-unit-cost-status><option value="payable" @selected(old('status', 'payable') === 'payable')>Still payable</option><option value="paid" @selected(old('status') === 'paid')>Already paid</option></select></div></label>
                                    <div data-unit-cost-account hidden>@include('partials.financial-account-select', ['accounts' => $selectedUnitFinancialAccounts, 'id' => 'new_cost_financial_account_id', 'label' => 'Paid from account'])</div>
                            ...
                            <section class="unit-ledger-panel">
                                ...
                                <div class="unit-cost-list">
                                    @forelse($selectedUnit->costs->take(20) as $cost)
                                        <article><span class="unit-cost-icon"><x-fa-icon :name="$cost->classification === 'capital' ? 'plus' : 'minus'" /></span><div><strong>{{ $cost->categoryLabel() }}</strong><small>{{ $cost->incurred_on->format('M j, Y') }}{{ $cost->vendor_name ? ' · '.$cost->vendor_name : '' }}</small>@if($cost->notes)<p>{{ $cost->notes }}</p>@endif</div><div><b>₱{{ number_format((float) $cost->amount, 2) }}</b><span class="booking-status {{ $cost->status === 'paid' ? 'status-confirmed' : 'status-payment_submitted' }}">{{ ucfirst($cost->status) }}</span></div>@if($cost->status === 'payable')<form method="POST" action="{{ route('sales.units.costs.paid', [$selectedUnit, $cost]) }}" class="unit-cost-payment-form">@csrf @method('PATCH')@if($selectedUnitFinancialAccounts->isNotEmpty())<select name="financial_account_id" required aria-label="Account used to pay {{ $cost->categoryLabel() }}"><option value="">Paid from account</option>@foreach($selectedUnitFinancialAccounts as $financialAccount)<option value="{{ $financialAccount->id }}">{{ $financialAccount->displayLabel() }}</option>@endforeach</select><button class="button button-ghost button-small" type="submit">Mark paid</button>@else<a class="financial-account-required compact" href="{{ route('accounting.index').'#financial-accounts' }}">Add account first</a>@endif</form>@endif</article>
                                    ...
                                    @forelse($selectedUnit->obligations as $obligation)
                                        ...
                                        <article>
                                            ...
                                            @if($obligation->status === 'active')
                                                <form method="POST" action="{{ route('sales.units.obligations.payments.store', [$selectedUnit, $obligation]) }}">@csrf<input name="installment_month" type="month" value="{{ old('installment_month', $asOfMonth->format('Y-m')) }}" required><div class="money-input"><span>₱</span><input name="amount" inputmode="decimal" value="{{ number_format((float) $obligation->monthly_amount, 2, '.', ',') }}" required data-accounting-input></div>@if($selectedUnitFinancialAccounts->isNotEmpty())<select name="financial_account_id" required aria-label="Account used for {{ $obligation->name }}"><option value="">Paid from account</option>@foreach($selectedUnitFinancialAccounts as $financialAccount)<option value="{{ $financialAccount->id }}">{{ $financialAccount->displayLabel() }}</option>@endforeach</select><button class="button button-primary button-small" type="submit">Record paid month</button>@else<a class="financial-account-required compact" href="{{ route('accounting.index').'#financial-accounts' }}">Add account first</a>@endif</form>
                                            @endif
                                        </article>
