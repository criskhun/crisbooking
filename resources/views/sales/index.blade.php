        ...
        <main class="dashboard-main">
            ...
            <section class="sales-dashboard">
                ...
                <section class="sales-ledger-card">
                    ...
                    <div class="sales-ledger-table"><div class="sales-ledger-head"><span>Booking</span><span>Client</span><span>Category</span><span>Status</span><span>Sale value</span></div>@forelse($recentBookings as $sale)<a href="{{ route('bookings.show', $sale) }}"><span><strong>#{{ $sale->id }} · {{ $sale->unit->name }}</strong><small>{{ $sale->start_at->format('M j, Y · g:i A') }}</small></span><span>{{ $sale->client->name }}</span><span>{{ str($sale->unit->category)->replace('_', ' ')->title() }}</span><span><em class="booking-status status-{{ $sale->status }}">{{ ucfirst($sale->status) }}</em></span><strong>{{ $sale->status === 'confirmed' ? '₱'.number_format($sale->revenueAmount(), 2) : '—' }}</strong></a>@empty<div class="overview-empty"><strong>No booking records in this category.</strong></div>@endforelse</div>
                </section>
            </section>
        </main>
    </div>
