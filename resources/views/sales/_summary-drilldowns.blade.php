            ...
            <div class="sales-summary-list sales-summary-action-list">
                ...
                @foreach($payableCostRows as $row)
                    ...
                    <article>
                        ...
                        <form method="POST" action="{{ route('sales.units.costs.paid', [$row['unit'], $row['cost']]) }}">@csrf @method('PATCH')@if($costAccounts->isNotEmpty())<select name="financial_account_id" required aria-label="Account used for {{ $row['cost']->categoryLabel() }}"><option value="">Paid from account</option>@foreach($costAccounts as $financialAccount)<option value="{{ $financialAccount->id }}">{{ $financialAccount->selectionLabel() }}</option>@endforeach</select><button class="button button-primary button-small" type="submit">Mark paid</button>@else<a class="financial-account-required compact" href="{{ route('accounting.index').'#financial-accounts' }}">Add account first</a>@endif<a href="{{ route('sales.index', array_filter(['unit' => $row['unit']->id, 'month' => $reportMonth?->format('Y-m')])) }}">Open unit</a></form>
                    </article>
                @endforeach
