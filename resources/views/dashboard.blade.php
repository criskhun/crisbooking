            ...
            @else
                <section class="host-overview">
                    ...
                    <section class="overview-section">
                        ...
                        <div class="overview-booking-list host-booking-list">@forelse($upcomingBookings as $booking)<a href="{{ route('bookings.show', $booking) }}"><time>{{ $booking->start_at->format('M j') }}<small>{{ $booking->start_at->format('g:i A') }}</small></time><span><strong>{{ $booking->unit->name }}</strong><small>{{ $booking->isManualBooking() ? $booking->sourceDisplayLabel().': '.$booking->customerDisplayName().' · '.$booking->durationDays().' '.Str::plural('day', $booking->durationDays()) : 'Client: '.$booking->customerDisplayName().' · '.$booking->party_size.' '.Str::plural('person', $booking->party_size) }}</small></span><span class="booking-status status-{{ $booking->status }}">{{ $booking->statusLabel() }}</span><b>₱{{ number_format($booking->total_amount, 2) }}</b></a>@empty<div class="overview-empty compact"><strong>No upcoming rentals.</strong><p>Your approved and pending requests will appear here.</p></div>@endforelse</div>
                    </section>
                </section>
            @endif
        </main>
