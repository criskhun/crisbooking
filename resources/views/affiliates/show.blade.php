            ...
            <div class="affiliate-detail-layout">
                <div>
                    ...
                    @if($affiliate->isAccepted())
                        ...
                        <section class="overview-section"><div class="overview-section-heading"><div><span class="eyebrow">Sales ledger</span><h2>Referred bookings</h2></div></div><div class="affiliate-sales-list">@forelse($affiliate->bookings as $booking)<article><span><strong>{{ $booking->unit->name }}</strong><small>{{ $booking->client->name }} · {{ $booking->created_at->format('M j, Y') }}</small></span><span class="booking-status status-{{ $booking->status }}">{{ ucfirst($booking->status) }}</span><b>{{ $booking->status === 'confirmed' ? '₱'.number_format($booking->affiliate_commission_amount, 2) : 'Pending' }}</b></article>@empty<div class="overview-empty compact"><strong>No referred booking yet.</strong><p>Bookings that begin through the tracked links will appear here.</p></div>@endforelse</div></section>
                    @endif
                </div>
