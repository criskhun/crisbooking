            ...
            <div class="affiliate-detail-layout">
                <div>
                    ...
                    @if($affiliate->isAccepted())
                        <section class="overview-section"><div class="overview-section-heading"><div><span class="eyebrow">Tracked links</span><h2>Assigned listings ready to share</h2><p>Only listings assigned by the host appear here and accept this affiliate’s referral code.</p></div></div><div class="affiliate-link-list">@forelse($affiliate->units->where('is_active', true) as $unit)<article>@if($unit->primaryImagePath())<img src="{{ Storage::disk('public')->url($unit->primaryImagePath()) }}" alt="">@else<span class="affiliate-link-placeholder"><x-category-icon :category="$unit->category" /></span>@endif<div><small>{{ str($unit->category)->replace('_', ' ')->title() }}</small><strong>{{ $unit->name }}</strong><input value="{{ $unit->publicUrl($affiliate->referral_code) }}" readonly></div><button type="button" data-affiliate-copy="{{ $unit->publicUrl($affiliate->referral_code) }}">Copy link</button><a href="{{ $unit->publicUrl($affiliate->referral_code) }}" target="_blank" rel="noopener">Preview</a></article>@empty<div class="overview-empty"><strong>No active listing is assigned.</strong><p>The host can update assignments in the management form above.</p></div>@endforelse</div></section>

                        <section class="overview-section"><div class="overview-section-heading"><div><span class="eyebrow">Sales ledger</span><h2>Referred bookings</h2></div></div><div class="affiliate-sales-list">@forelse($affiliate->bookings as $booking)<article><span><strong>{{ $booking->unit->name }}</strong><small>{{ $booking->customerDisplayName() }} · {{ $booking->isManualBooking() ? $booking->sourceDisplayLabel().' · ' : '' }}{{ $booking->created_at->format('M j, Y') }}</small></span><span class="booking-status status-{{ $booking->status }}">{{ $booking->statusLabel() }}</span><b>{{ $booking->status === 'confirmed' ? '₱'.number_format($booking->affiliate_commission_amount, 2) : $booking->statusLabel() }}</b></article>@empty<div class="overview-empty compact"><strong>No referred booking yet.</strong><p>Bookings that begin through the tracked links or outside entries will appear here.</p></div>@endforelse</div></section>
                        @php
                        ...
                        <section class="overview-section affiliate-partnership-review">
                            ...
                            @if($affiliateRating)
                                <div class="review-submitted"><strong><x-rating-stars :rating="$affiliateRating->rating" /></strong><p>{{ $affiliateRating->comment }}</p><small>Rated by {{ $affiliate->host->name }}</small></div>
                            @elseif($viewerCanRateAffiliate)
                                <form method="POST" action="{{ route('affiliates.reviews.store', $affiliate) }}" class="review-form">@csrf<fieldset><legend>Your rating</legend><div class="review-star-options">@foreach(range(5, 1) as $star)<input id="affiliate-rating-{{ $star }}" name="rating" type="radio" value="{{ $star }}" required><label for="affiliate-rating-{{ $star }}"><x-fa-icon name="star" /></label>@endforeach</div></fieldset><div class="field-group"><label for="affiliate_review_comment">Public comment</label><textarea id="affiliate_review_comment" name="comment" rows="3" minlength="10" maxlength="1500" required></textarea></div><button class="button button-primary" type="submit">Publish affiliate review</button></form>
                            @else
