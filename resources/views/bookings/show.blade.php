                        ...
                        @if($canManageFinances && $booking->status === 'confirmed')
                            <details class="booking-extension-editor" @if($extensionHasErrors) open @endif>
                                ...
                                <form method="POST" action="{{ route('bookings.extensions.store', $booking) }}" class="booking-extension-form" data-booking-extension-form data-current-end="{{ $booking->end_at->format('Y-m-d\TH:i:s') }}">
                                    ...
                                    <div class="field-group"><label for="extension_payment_status">Payment status</label><select id="extension_payment_status" name="payment_status" required data-extension-payment-status><option value="paid" @selected(old('payment_status') === 'paid')>Paid now</option><option value="collectible" @selected(old('payment_status', 'collectible') === 'collectible')>Add to collectibles</option></select>@error('payment_status')<p class="error-text">{{ $message }}</p>@enderror</div>
                                    <div class="field-group booking-extension-account" data-extension-financial-account hidden>@include('partials.financial-account-select', ['accounts' => $financialAccounts, 'id' => 'extension_financial_account_id', 'label' => 'Account that received payment'])</div>
                    ...
                    <section class="booking-finance-card">
                        ...
                        @if($canManageFinances)
                            <div class="booking-finance-forms">
                                <form method="POST" action="{{ route('bookings.financial-entries.store', $booking) }}">@csrf<input type="hidden" name="kind" value="payment"><input type="hidden" name="category" value="balance_payment"><strong>Record a collection</strong><label><span>Amount</span><div class="money-input"><span>₱</span><input name="amount" type="text" inputmode="decimal" required data-accounting-input></div></label>@include('partials.financial-account-select', ['accounts' => $financialAccounts, 'id' => 'collection_financial_account_id', 'label' => 'Account that received it'])<label><span>Reference or note</span><input name="notes" maxlength="500" placeholder="Cash, transfer reference…"></label><button class="button button-primary button-small" type="submit" @disabled($financialAccounts->isEmpty())>Add payment</button></form>
                                ...
                                <form method="POST" action="{{ route('bookings.financial-entries.store', $booking) }}" data-deposit-action-form>@csrf<strong>Security deposit</strong><label><span>Action</span><select name="kind" required data-deposit-action-kind><option value="deposit">Record deposit collected</option><option value="deposit_refund">Return deposit</option><option value="deposit_application">Apply deposit to charges</option></select></label><label><span>Amount</span><div class="money-input"><span>₱</span><input name="amount" type="text" inputmode="decimal" required data-accounting-input></div></label><span data-deposit-account-field>@include('partials.financial-account-select', ['accounts' => $financialAccounts, 'id' => 'deposit_financial_account_id', 'label' => 'Account used'])</span><label><span>Note</span><input name="notes" maxlength="500" placeholder="Return method or damage reference"></label><button class="button button-primary button-small" type="submit">Record deposit action</button></form>
                            </div>
                        @endif
                    </section>
                @endif
                            ...
                            @forelse($booking->expenses as $expense)
                                <article class="booking-expense-entry status-{{ $expense->status }}">
                                    ...
                                    @if($expense->provider_user_id && $expense->status === 'completed')
                                        <form method="POST" action="{{ route('bookings.expenses.status', [$booking, $expense]) }}" enctype="multipart/form-data" class="expense-payment-form">@csrf @method('PATCH')<input type="hidden" name="status" value="paid"><label><span>Proof of payment</span><input type="file" name="payment_proof" accept="image/jpeg,image/png,image/webp,application/pdf" required></label>@include('partials.financial-account-select', ['accounts' => $financialAccounts, 'id' => 'expense_account_'.$expense->id, 'label' => 'Paid from account'])@error('payment_proof')<p class="error-text">{{ $message }}</p>@enderror<button class="button button-primary button-small" type="submit" @disabled($financialAccounts->isEmpty())>Mark paid</button></form>
