    ...
    <div class="dashboard-shell">
        ...
        <main class="dashboard-main">
            ...
            <section class="public-profile-card">
                <div class="public-profile-hero">@include('partials.avatar', ['avatarUser' => $profileUser, 'avatarClass' => 'public-profile-avatar'])<div><small>{{ ucfirst($profileUser->role) }} profile</small><h2>{{ $profileUser->name }}</h2><p>{{ $profileUser->city }}, {{ $profileUser->nationality }}</p></div><strong>✓ Identity details complete</strong></div>
                ...
                <section class="profile-reviews-section">
                    ...
                    @if ($reviewSummaries->isNotEmpty())<div class="profile-rating-summary">@foreach(['host' => 'As a host', 'client' => 'As a client', 'affiliate' => 'As an affiliate'] as $context => $label)@if($reviewSummaries->has($context))<article><small>{{ $label }}</small><strong>★ {{ number_format($reviewSummaries[$context]['average'], 1) }}</strong><span>{{ $reviewSummaries[$context]['count'] }} {{ Str::plural('review', $reviewSummaries[$context]['count']) }}</span></article>@endif @endforeach</div>@endif
                    <div class="profile-review-list">@forelse($profileUser->reviewsReceived as $review)<article><div>@include('partials.avatar', ['avatarUser' => $review->reviewer, 'avatarClass' => 'reviewer-avatar'])<div><strong>{{ $review->reviewer->name }}</strong><small>{{ str($review->reviewee_context)->title() }} review · {{ $review->created_at->format('M j, Y') }}</small></div><b>{{ str_repeat('★', $review->rating) }}{{ str_repeat('☆', 5 - $review->rating) }}</b></div><p>{{ $review->comment }}</p>@if($review->booking)<small>Booking: {{ $review->booking->unit->name }}</small>@else<small>Affiliate partnership review</small>@endif</article>@empty<div class="overview-empty compact"><strong>No reviews yet.</strong><p>Reviews appear after completed confirmed bookings or accepted affiliate partnerships.</p></div>@endforelse</div>
                </section>
