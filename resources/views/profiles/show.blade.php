        ...
        <main class="dashboard-main">
            ...
            <section class="public-profile-card">
                ...
                <div class="profile-validation-grid">
                    ...
                    <section><span class="eyebrow">Contact</span><dl><div><dt>Email</dt><dd>{{ $profileUser->email }}</dd></div><div><dt>Mobile</dt><dd>{{ $profileUser->phone }} {{ $profileUser->phone_verified_at ? '✓' : '' }}</dd></div><div><dt>Address</dt><dd>{{ collect([$profileUser->address, $profileUser->barangay, $profileUser->city, $profileUser->province, $profileUser->country])->filter()->join(', ') }}</dd></div></dl></section>
                    <section><span class="eyebrow">Identity check</span><dl><div><dt>ID type</dt><dd>{{ str($profileUser->government_id_type)->replace('_', ' ')->title() }}</dd></div><div><dt>ID number</dt><dd>Ending in {{ substr($profileUser->government_id_number, -4) }}</dd></div><div><dt>Date of birth</dt><dd>{{ $profileUser->date_of_birth?->format('M j, Y') }}</dd></div></dl><a class="profile-document-link" href="{{ route('profiles.document', $profileUser) }}" target="_blank">View private ID document ↗</a></section>
