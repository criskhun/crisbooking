        ...
        <main class="dashboard-main">
            ...
            <section class="public-profile-card">
                ...
                <section class="profile-reviews-section">
                    ...
                    <div class="profile-review-list">@forelse($profileUser->reviewsReceived as $review)<article><div><span>{{ strtoupper(substr($review->reviewer->name, 0, 1)) }}</span><div><strong>{{ $review->reviewer->name }}</strong><small>{{ str($review->reviewee_context)->title() }} review · {{ $review->created_at->format('M j, Y') }}</small></div><b>{{ str_repeat('★', $review->rating) }}{{ str_repeat('☆', 5 - $review->rating) }}</b></div><p>{{ $review->comment }}</p>@if($review->booking)<small>Booking: {{ $review->booking->unit->name }}</small>@else<small>Affiliate partnership review</small>@endif</article>@empty<div class="overview-empty compact"><strong>No reviews yet.</strong><p>Reviews appear after completed confirmed bookings or accepted affiliate partnerships.</p></div>@endforelse</div>
                </section>
