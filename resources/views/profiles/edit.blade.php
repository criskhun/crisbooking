                ...
                <form method="POST" action="{{ route('profile.update') }}" enctype="multipart/form-data" class="verification-form" data-verification-form data-location-provinces-url="{{ route('profile.locations.provinces') }}" data-location-cities-url="{{ route('profile.locations.cities') }}" data-location-barangays-url="{{ route('profile.locations.barangays') }}">
                    ...
                    <section>
                        ...
                        <div class="verification-grid">
                            ...
                            <div class="field-group"><label for="country">Country</label><div class="searchable-combobox" data-address-combobox><input id="country" name="country" type="search" data-options-id="country-options" autocomplete="off" placeholder="Search country…" value="{{ old('country', $profileUser->country ?: 'Philippines') }}" required data-country-input><span aria-hidden="true">⌄</span><datalist id="country-options">@foreach ($countries as $code => $country)<option value="{{ $country }}" data-code="{{ $code }}"></option>@endforeach</datalist></div>@error('country')<p class="error-text">{{ $message }}</p>@enderror</div>
                            ...
                            <div class="field-group"><label for="barangay">Barangay</label><div class="searchable-combobox" data-address-combobox><input id="barangay" name="barangay" type="search" data-options-id="barangay-options" autocomplete="off" placeholder="Select city first" value="{{ old('barangay', $profileUser->barangay) }}" required data-barangay-input><span aria-hidden="true">⌄</span><datalist id="barangay-options"></datalist></div><small class="field-help" data-location-status aria-live="polite">Choose a country to load address suggestions.</small>@error('barangay')<p class="error-text">{{ $message }}</p>@enderror</div>
