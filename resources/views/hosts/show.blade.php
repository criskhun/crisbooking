...
@section('content')
    ...
    <main class="host-storefront-shell">
        <section class="host-storefront-hero" data-guide-feature="host-storefront">
            ...
            <dl><div><dt>Active listings</dt><dd>{{ $hostUser->units->count() }}</dd></div><div><dt>Host rating</dt><dd>@if($hostRating)<x-fa-icon name="star" class="fa-rating" /> {{ number_format($hostRating, 1) }}@else New host @endif</dd></div><div><dt>Location</dt><dd>{{ $hostUser->city ?: 'Davao Region' }}</dd></div></dl>
        </section>
        ...
        <section class="host-storefront-listings">
            ...
            <div class="host-listing-grid">
                @forelse($hostUser->units as $unit)
                    @php($startingPrice = $unit->startingPrice())
                    <article>
                        <div class="host-listing-media">@include('partials.listing-card-carousel', ['carouselUnit' => $unit, 'carouselLinkClass' => 'host-listing-photo'])@if($unit->hasSale())<span class="listing-sale-badge">{{ number_format((float) $unit->sale_percentage, 0) }}% off</span>@endif @include('partials.listing-favorite', ['favoriteUnit' => $unit])</div>
                        <div><small>{{ str($unit->category)->replace('_', ' ')->title() }} · {{ $unit->location ?: 'Location arranged' }}</small><h3>{{ $unit->name }}</h3><span class="host-listing-rating">@if($unit->listing_reviews_count)<x-fa-icon name="star" class="fa-rating" /> {{ number_format((float) $unit->listing_reviews_avg_rating, 1) }} ({{ $unit->listing_reviews_count }})@else New listing @endif</span><p>{{ Str::limit($unit->description ?: 'Open the listing for full details, rates, and rules.', 110) }}</p><footer><span>From @if($unit->hasSale())<del>₱{{ number_format($startingPrice, 2) }}</del>@endif <strong>₱{{ number_format($unit->discountedPrice($startingPrice), 2) }}</strong></span><a class="button button-primary" href="{{ route('listings.show', $unit) }}">View & book</a></footer></div>
                    </article>
                @empty
