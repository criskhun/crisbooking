        ...
        <section class="host-storefront-listings">
            ...
            <div class="host-listing-grid">
                @forelse($hostUser->units as $unit)
                    ...
                    <article>
                        <div class="host-listing-media"><a class="host-listing-photo" href="{{ route('listings.show', $unit) }}">@if($unit->primaryImagePath())<img src="{{ Storage::disk('public')->url($unit->primaryImagePath()) }}" alt="{{ $unit->name }}">@else<span>{{ ['car' => '🚗', 'condo' => '🏢', 'cleaning' => '🧹', 'driving' => '🛞', 'massage' => '💆', 'consultancy' => '💬'][$unit->category] ?? '◇' }}</span>@endif</a>@if($unit->hasSale())<span class="listing-sale-badge">{{ number_format((float) $unit->sale_percentage, 0) }}% off</span>@endif @include('partials.listing-favorite', ['favoriteUnit' => $unit])</div>
                        <div><small>{{ str($unit->category)->replace('_', ' ')->title() }} · {{ $unit->location ?: 'Location arranged' }}</small><h3>{{ $unit->name }}</h3><span class="host-listing-rating">{{ $unit->listing_reviews_count ? '★ '.number_format((float) $unit->listing_reviews_avg_rating, 1).' ('.$unit->listing_reviews_count.')' : 'New listing' }}</span><p>{{ Str::limit($unit->description ?: 'Open the listing for full details, rates, and rules.', 110) }}</p><footer><span>From @if($unit->hasSale())<del>₱{{ number_format($startingPrice, 2) }}</del>@endif <strong>₱{{ number_format($unit->discountedPrice($startingPrice), 2) }}</strong></span><a class="button button-primary" href="{{ route('listings.show', $unit) }}">View & book</a></footer></div>
                    </article>
                @empty
